    ...
    if (code && state) {
      try {
        ...
        if (!userResponse.ok) {
          ...
          throw new Error(`User data fetch failed: ${errorData.error?.message || 'Unknown error'}`);
        }

        const userData = await userResponse.json();
        console.log('User data received:', { id: userData.id, name: userData.name });
        ...
        // Get user's pages
        const pagesResponse = await fetch(`https://graph.facebook.com/v18.0/me/accounts?fields=id,name,category,access_token,instagram_business_account&access_token=${accessToken}`);
